...
do
    local r, sev = run(
        "<4>Jun  3 16:32:52 DreamMachinePro DreamMachinePro kernel: [quic_sm_reassemble_func#1025]: failed to allocate reassemble cont."
    )
...
do
    local r = run(
        "<28>Jun  3 16:32:54 DreamMachinePro DreamMachinePro systemd[1]: udapi-server.service: Got notification message from PID 470652, but reception only permitted for main PID 2193"
    )
    check(
        "systemd.log",
        r.log,
        "udapi-server.service: Got notification message from PID 470652, but reception only permitted for main PID 2193"
    )
...
do
    local r, sev = run(
        "<30>Jun  3 16:33:07 AccessPointnanoHD e063daccbd48,UAP-nanoHD-6.7.41+15623: mcad: mcad[21498]: wireless_agg_stats.log_sta_anomalies(): bssid=e2:63:da:ac:bd:4a radio=rai0 vap=rai2 sta=c2:5b:04:93:bb:fe satisfaction_now=70 anomalies=low_phy_rate"
    )
    ...
    check(
        "ap.log",
        r.log,
        "mcad: mcad[21498]: wireless_agg_stats.log_sta_anomalies(): bssid=e2:63:da:ac:bd:4a radio=rai0 vap=rai2 sta=c2:5b:04:93:bb:fe satisfaction_now=70 anomalies=low_phy_rate"
    )
...
do
    ...
    check(
        "cef.log",
        r.log,
        "esphome-somfy 4c:18 connected to Canards IoT on UAP-IW-HD. Connection Info: Ch. 11 (2.4 GHz, 20 MHz), -73 dBm. IP: 10.123.4.13"
    )
...
do
    local r = run(
        "<31>Jun  3 10:00:00 USW-Pro a1b2c3d4e5f6,USW-Pro-24-PoE-7.0.50: hostapd: nonsense without colon delimiter elsewhere"
    )
