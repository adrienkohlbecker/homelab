...
def test_passt_native_backend_uses_passt_netdev(
    ...
    cmd = m._boot_command()
    netdev = cmd[cmd.index("-netdev") + 1]
    assert netdev.startswith("passt,id=net0,quiet=on")
    assert "stream," not in netdev
    assert "addr.path=" not in netdev
    assert "user,id=user.0" not in netdev
...
def test_passt_netdev_params_mirror_sidecar_ports(
    ...
    params = m._passt_netdev_params()

    # Comma-list value carried as one option via ,,-escaping (single comma
    # would be read as an option separator by qemu).
    assert "tcp-ports=127.0.0.1/2222:22,,3000:32400" in params
    assert "udp-ports=127.0.0.1/4000:51820" in params
    # box is in the topology -> address pinned, mirroring the sidecar flags.
    assert ",address=10.234." in f",{params}"
    assert "netmask=" in params
    ...
    assert f"tcp-ports={tcp_spec.replace(',', ',,')}" in params
    assert f"udp-ports={udp_spec.replace(',', ',,')}" in params
...
def test_passt_netdev_params_skip_address_off_topology(
    ...
    # UDP forwards are empty here too -> no udp-ports key, but tcp (SSH) stays.
    assert "tcp-ports=" in params
