...
The function reads `roles/<role>/tasks/_setup.yml` relative to CWD, so each
...
def _stage_role(root: Path, role: str, body: str | None) -> None:
    ...
    if body is not None:
        (role_dir / "_setup.yml").write_text(body)


def test_returns_true_when_setup_yml_imports_podman(
...
def test_returns_true_when_setup_yml_imports_nginx(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _stage_role(
        tmp_path,
...
def test_returns_false_when_setup_yml_imports_neither(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _stage_role(
        tmp_path,
...
def test_returns_false_when_setup_yml_missing(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _stage_role(tmp_path, "no_setup", body=None)
    monkeypatch.chdir(tmp_path)
    assert machine.is_service_role("no_setup") is False
