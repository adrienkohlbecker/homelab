...
class TestHelpLineRegex:
    def test_flag_only(self) -> None:
        line = "      --rm                           Automatically remove the container and"
        m = epi._HELP_LINE.match(line)
        assert m is not None
        assert m.group(2) == "--rm"
        assert m.group(3) is None

    def test_flag_with_short(self) -> None:
        line = "  -d, --detach                       Run container in background"
        m = epi._HELP_LINE.match(line)
        assert m is not None
        assert m.group(1) == "d"
        assert m.group(2) == "--detach"
        assert m.group(3) is None

    def test_flag_with_value(self) -> None:
        line = "      --name string                  Assign a name to the container"
        m = epi._HELP_LINE.match(line)
        assert m is not None
        assert m.group(2) == "--name"
        assert m.group(3) is not None

    def test_flag_with_short_and_value(self) -> None:
        line = "  -v, --volume strings               Bind mount a volume into the container"
        m = epi._HELP_LINE.match(line)
        assert m is not None
        assert m.group(1) == "v"
        assert m.group(2) == "--volume"
        assert m.group(3) is not None

    def test_non_help_line(self) -> None:
        assert epi._HELP_LINE.match("Run a command in a new container") is None
        assert epi._HELP_LINE.match("") is None
