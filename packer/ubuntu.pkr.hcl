packer {
  required_plugins {
    vmware = {
      version = "~> 1"
      source = "github.com/hashicorp/vmware"
    }
    vagrant = {
      source  = "github.com/hashicorp/vagrant"
      version = "~> 1"
    }
  }
}

source "vmware-iso" "ubuntu" {
  version = 21

  cpus                  = 2
  memory                = 2048

  guest_os_type         = "arm-ubuntu-64"
  iso_checksum          = "sha256:74b8a9f71288ae0ac79075c2793a0284ef9b9729a3dcf41b693d95d724622b65"
  iso_url               = "https://cdimage.ubuntu.com/releases/22.04.4/release/ubuntu-22.04.4-live-server-arm64.iso"

  disk_size         = 8192
  disk_adapter_type = "nvme"

  usb = "true"

  ssh_username          = "vagrant"
  ssh_private_key_file  = "${path.root}/vagrant.key"
  ssh_timeout           = "20m"

  network_adapter_type = "vmxnet3"

  output_directory = "${path.root}/output-${source.name}"
  http_directory = "${path.root}/http"

  boot_wait             = "3s"
  boot_command = [
    "e<wait>",
    "<down><down><down>",
    "<end><bs><bs><bs><bs><bs><bs><bs><bs><bs><wait>",
    "autoinstall ds=nocloud-net\\;s=http://{{ .HTTPIP }}:{{ .HTTPPort }}/ ---<wait>",
    "<f10><wait>"
  ]
  shutdown_command      = "sudo shutdown now"

  vmx_data = {
    "disk.EnableUUID" = "true"
    "usb_xhci.present" = "true"
    "keyboardAndMouseProfile" = "52b6ca63-f634-a2dc-88f6-220e6fb7dfd5" # default mac layout
  }
}

source "vmware-vmx" "ubuntu-box" {
  source_path = "${path.root}/vmx-ubuntu/packer-ubuntu.vmx"

  disk_additional_size = [ 8192 ]

  output_directory = "${path.root}/output-${source.name}"

  ssh_username          = "vagrant"
  ssh_private_key_file  = "${path.root}/vagrant.key"
  ssh_timeout           = "5m"

  shutdown_command      = "sudo shutdown now"

  # See https://github.com/hashicorp/packer-plugin-vmware/issues/119
  vmx_data = {
    "nvme0:1.filename" = "disk-1.vmdk"
    "nvme0:1.present" = "TRUE"
  }
}

source "vmware-vmx" "ubuntu-lab" {
  source_path = "${path.root}/vmx-ubuntu/packer-ubuntu.vmx"

  disk_additional_size = [ 8192, 8192, 8192, 1024, 1024, 1024, 1024, 1024, 1024 ]

  output_directory = "${path.root}/output-${source.name}"

  ssh_username          = "vagrant"
  ssh_private_key_file  = "${path.root}/vagrant.key"
  ssh_timeout           = "5m"

  shutdown_command      = "sudo shutdown now"

  # See https://github.com/hashicorp/packer-plugin-vmware/issues/119
  vmx_data = {
    "nvme0:1.filename" = "disk-1.vmdk"
    "nvme0:1.present" = "TRUE"
    "nvme0:2.filename" = "disk-2.vmdk"
    "nvme0:2.present" = "TRUE"
    "nvme0:3.filename" = "disk-3.vmdk"
    "nvme0:3.present" = "TRUE"
    "sata0:0.filename" = "disk-4.vmdk"
    "sata0:0.present" = "TRUE"
    "sata0:1.filename" = "disk-5.vmdk"
    "sata0:1.present" = "TRUE"
    "sata0:2.filename" = "disk-6.vmdk"
    "sata0:2.present" = "TRUE"
    "sata0:3.filename" = "disk-8.vmdk" # disk-7 is not created?
    "sata0:3.present" = "TRUE"
    "sata0:4.filename" = "disk-9.vmdk"
    "sata0:4.present" = "TRUE"
    "sata0:5.filename" = "disk-10.vmdk"
    "sata0:5.present" = "TRUE"
  }
}

build {
  sources = ["source.vmware-iso.ubuntu", "source.vmware-vmx.ubuntu-lab", "source.vmware-vmx.ubuntu-box"]

  provisioner "file" {
    except = ["source.vmware-iso.ubuntu"]
    source = "${path.root}/scripts/"
    destination = "/home/vagrant/"
  }
  provisioner "shell" {
    except = ["source.vmware-iso.ubuntu"]
    inline = ["chmod +x /home/vagrant/*.sh", "sudo -HE /home/vagrant/provision.sh"]
  }

  post-processors {
    post-processor "manifest" {
      output     = "${path.root}/output-${source.name}/manifest.json"
    }

    post-processor "shell-local" {
      except = ["source.vmware-iso.ubuntu"]
      inline_shebang = "/bin/bash"
      inline = [
          "set -euxo pipefail",
          "sed -i.bak 's/nvme0:0.present = \"TRUE\"/nvme0:0.present = \"FALSE\"/' ${path.root}/output-${source.name}/*.vmx",
          "truncate -s0 ${path.root}/output-${source.name}/disk-cl1*.vmdk"
      ]
    }

    post-processor "shell-local" {
      only = ["source.vmware-iso.ubuntu"]
      inline_shebang = "/bin/bash"
      inline = [
          "set -euxo pipefail",
          "mkdir -p ${path.root}/vmx-${source.name}",
          "rm -rf ${path.root}/vmx-${source.name}/*",
          "jq \".builds[].files[].name\" ${path.root}/output-${source.name}/manifest.json | xargs -I {} cp -v {} ${path.root}/vmx-${source.name}"
      ]
    }

    post-processor "vagrant" {
      vagrantfile_template = "${path.root}/Vagrantfile.tmpl"
      output = "${path.root}/packer_{{.BuildName}}_{{.Provider}}_{{.Architecture}}.box"
    }
  }
}


// , "source.vmware-vmx.ubuntu-lab"
