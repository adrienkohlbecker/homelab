...
- **Base machine install** (`hosts: box,lab,pug,fox`) — sub-bands: *host base* (OS, networking, access), *persistent state* (`services` before its `ssh`/`ssh_root` identity consumers), *storage & boot* (`zfs`/`zfs_autobackup`/`zfsbootmenu`/`refind`), *service platform* (`podman`, `certbot`, `nginx`), *observability* (`csplogger`/`netdata`/`fluentbit`). Roles within host-base are mutually independent.
