...
- **`sqlite_dataset`** — `tasks_from: dataset`, `sqlite_dataset_args: { name, owner, group }` → creates `/mnt/services/sqlite/<name>/` and symlinks each `links[i]` dest into it, placing SQLite DBs on the dedicated 4K-recordsize ZFS dataset. Optional keys: `mode` (dir, default `0750`), `file_mode` (pre-created files, default `0640`), `condition` (check-mode gate), `pre_create` (plant empty src files for apps that migrate an empty DB — jellyfin, the \*arr stack), `force` (plant the symlink even over a dangling target or existing file — for apps that create their own DB on cold start, e.g. kuma/sabnzbd/tautulli; mutually exclusive with `pre_create` since it drops the dest-clobber safety), `links` (list of dest paths; basenames must be unique). By default — `state: link` without `force:` — the helper refuses to convert a real DB already at a dest into a symlink (snapshot-restore safety). Canonical callers: [roles/jellyfin/tasks/main.yml](roles/jellyfin/tasks/main.yml) (pre_create), [roles/kuma/tasks/main.yml](roles/kuma/tasks/main.yml) (force).
