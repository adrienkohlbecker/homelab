...
- **op:// env refs only resolve under `op run --`.** Toml-defined tasks wrap explicitly (`[tasks.tf] run = 'op run -- tofu "$@"'`); file-based tasks under `mise-tasks/<group>/<name>` do **not** — mise exports the literal `op://Lab/.../password` string into the script's env. A consumer that treats it as the real secret fails with auth errors (HA returned 401 on `Authorization: Bearer op://...`). Fix: re-exec the script under `op run --` behind a guard env var to avoid an infinite loop. Canonical: the preamble at the top of [mise-tasks/ha/sync](mise-tasks/ha/sync). Diagnostic: log `len=${#FOO_TOKEN}` — if it equals the length of the `op://` string (~40 chars), resolution didn't happen.
