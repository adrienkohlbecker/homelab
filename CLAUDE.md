...
Everything HA reads from `/mnt/services/homeassistant/` that's user-authored — `automations.yaml`, `scripts.yaml`, `scenes.yaml` (GUI-written), plus `sensors.yaml`, `templates.yaml`, `input_numbers.yaml`, `custom_templates/macros.jinja`, and `blueprints/automation/*.yaml` (operator-authored) — lives in a separate GitHub repo ([adrienkohlbecker/homelab_ha_config](https://github.com/adrienkohlbecker/homelab_ha_config), private) registered as a git submodule at [roles/homeassistant/files/ha_gui_config/](roles/homeassistant/files/ha_gui_config). The submodule is the source of truth. The homeassistant role only creates the directories + touches the include-targets (automations/scripts/scenes/sensors/templates) so HA's loader doesn't error on a fresh host before the first sync. Every push parse-validates each changed file (YAML via PyYAML with HA tag stubs, .jinja via jinja2) and aborts the upload on any syntax error — so a broken file never reaches lab.
