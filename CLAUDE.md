...
`test/testrole.py <role>` boots `lab` (default) and applies the role end-to-end. `test/testall.py` fans out role × machine in parallel. Flags: `--machine {minimal,lab,pug}`, `--keep`, `testall.py --retry-failed`. Exit codes: `0` success, `1` converge, `124` timeout, `125` idempotence, `130` cancelled. Failed-run artifacts → `test/out/<machine>.<ubuntu>.<role>.*.ansi`. Packer artifact trees live under `/mnt/scratch/homelab_ci/<codename>/` (Linux) or `packer/artifacts/<codename>/` (Mac); `minimal` downloads and caches the upstream Ubuntu cloud image instead. macOS needs `xorriso` for its cloud-init seed ISO. On the macOS aarch64 fixture, run `mise run test:firmware` once during bootstrap — it fetches a newer edk2 (Homebrew's edk2-stable202408 ASSERTs in rEFInd across a warm reboot, wedging any role that reboots: reboot/kdump/console `_verify`). Worktrees symlink it from the main checkout, so one fetch covers all; the harness **errors with fetch guidance** if it is missing rather than falling back to the broken firmware.
...
GitLab CI ([.gitlab-ci.yml](.gitlab-ci.yml)) runs the role-test matrix as **qemu cells** on one of two infrastructures, selected by the single `HOMELAB_CI_TARGET` pipeline variable: `aws_qemu` (default — nested qemu/KVM on fleeting-autoscaled AWS shell hosts; cells hydrate promoted qemu images from S3 via OIDC) or `lab` (qemu on lab's local shell runner; cells boot the images straight from lab's `/mnt/scratch/homelab_ci` and skip hydration — no object store, no AWS egress for cells). The `.qemu_image` bake always runs on `lab-shell-qemu` (the sole KVM builder): it writes the artifacts into `HOMELAB_CI_DIR` for the lab cells **and** uploads each build to S3 so the aws_qemu cells hydrate the same promoted build. The promoted build for each `machine/ubuntu` is selected by a `promoted.json` pointer **object** inside the S3 image bucket (not SSM). `detect` ([mise-tasks/ci/detect.py](mise-tasks/ci/detect.py)) classifies the diff and writes a dynamic child pipeline — one `test_cells` job per `role:variant[:ubuntu]` cell — alongside `lint` and `unit_tests` (both on GitLab SaaS runners). `detect` renders on the qemu cell pool picked by `HOMELAB_CI_TARGET` via `$DETECT_RUNNER_TAG` — `lab-shell-qemu` for a manual lab-target pipeline, else `aws-shell-qemu` — so a lab run never depends on fox/AWS and the render warms the first cell host. GitLab-hosted runners handle stateless jobs, including `qemu_host_ami` and `ci_image`; KVM image builders remain on `lab-shell-qemu`. QEMU image uploads retain the current build plus three rollback builds with object tags; S3 lifecycle expires abandoned and superseded objects after seven days. Design: [notes/ci_aws_nested_qemu_cells.md](notes/ci_aws_nested_qemu_cells.md).

**Escalation:** `minimal:` or `pug:` in `meta/test.yml` `machines:` adds the corresponding specialized cell; `machines: {lab: {memory_mb: 5120}}` raises that cell's guest RAM above the `test/machine.py` default; `ubuntu:` lists add per-release cells. **Local-debug:** `CI_BASE_REF=HEAD~5 mise run ci:detect --child-path /tmp/cells.yml` previews the cell matrix (logged to stderr). CI secrets: [notes/runbooks/ci_secrets.md](notes/runbooks/ci_secrets.md).
