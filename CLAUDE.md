...
- **DO NOT use Ansible handlers for service restarts.** Handlers run at end-of-play, which doesn't compose with the `systemd_unit` helper's inline enable-then-start ordering (and the per-role image pull that must precede it). Drive restarts via the helper's inline OR chain on `*_result.changed` / `*_rotated`. See *Helper roles → systemd_unit*.
...
- Tasks depending on a freshly-created service user/group/dir must be gated on `not ansible_check_mode or <svc>_user.uid is defined` — a fresh check-mode run reports creation without exporting numeric IDs, while a proper run must not silently skip work when an expected fact is missing.
...
GitLab CI ([.gitlab-ci.yml](.gitlab-ci.yml)) on the off-fox orchestrator runs the role-test matrix as **qemu cells** on one of two infrastructures, selected by the single `HOMELAB_CI_TARGET` pipeline variable: `aws_qemu` (default — nested qemu/KVM on fleeting-autoscaled AWS shell hosts; cells hydrate promoted qemu images from S3 via OIDC) or `lab` (qemu on lab's local shell runner; cells boot the images straight from lab's `/mnt/scratch/homelab_ci` and skip hydration — no object store, no AWS egress for cells). The `.qemu_image` bake always runs on `lab-shell-qemu` (the sole KVM builder): it writes the artifacts into `HOMELAB_CI_DIR` for the lab cells **and** uploads each build to S3 so the aws_qemu cells hydrate the same promoted build. The promoted build for each `machine/ubuntu` is selected by a `promoted.json` pointer **object** inside the S3 image bucket (not SSM). `detect` ([mise-tasks/ci/detect.py](mise-tasks/ci/detect.py) `gitlab`) classifies the diff and writes a dynamic child pipeline — one `test_cells` job per `role:variant[:ubuntu]` cell — alongside `lint` and `unit_tests` (both on GitLab SaaS runners). `detect` renders on the qemu cell pool picked by `HOMELAB_CI_TARGET` via `$DETECT_RUNNER_TAG` — `lab-shell-qemu` for a manual lab-target pipeline, else `aws-shell-qemu` — so a lab run never depends on fox/AWS and the render warms the first cell host. Manual jobs run on the on-fox `fox-docker-aws` runner: image bakes (`qemu_host_ami`, `fox_image`, `zbm_build`, `ci_image`) and `qemu_image_reaper` (S3). Design: [notes/ci_aws_nested_qemu_cells.md](notes/ci_aws_nested_qemu_cells.md).

**Escalation:** `minimal:` in `meta/test.yml` `machines:` adds a cell; `ubuntu:` list adds per-release cells. **Local-debug:** `CI_BASE_REF=HEAD~5 uv run python mise-tasks/ci/detect.py gitlab --child-path /tmp/cells.yml` previews the cell matrix (logged to stderr). CI secrets: [notes/runbooks/ci_secrets.md](notes/runbooks/ci_secrets.md).
