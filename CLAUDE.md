...
- **op:// env refs only resolve under `op run --`.** Toml-defined tasks wrap explicitly (`[tasks.tf] run = 'op run -- tofu "$@"'`); file-based tasks under `mise-tasks/<group>/<name>.sh` do **not** — mise exports the literal `op://Lab/.../password` string into the script's env. A consumer that treats it as the real secret fails with auth errors (HA returned 401 on `Authorization: Bearer op://...`). Fix: re-exec the script under `op run --` behind a guard env var to avoid an infinite loop. Canonical: the preamble at the top of [mise-tasks/ha/sync.py](mise-tasks/ha/sync.py). Diagnostic: log `len=${#FOO_TOKEN}` — if it equals the length of the `op://` string (~40 chars), resolution didn't happen.
...
**Never set `no_log: true`.** It hides the rendered diff / task output that makes a failed converge debuggable — and its only upside (keeping secrets out of a log) buys nothing here: applies run interactively on the operator's own workstation (credentials in that terminal are fine) and prod applies are never captured to a file or CI log. So the secret-leak protection is moot while the debugging cost is real. This holds even for tasks that render or echo a vaulted value (`template:` of a secret-bearing config, a `--dump` that prints a password): let it show. Don't add `no_log` "just in case."

**Every bash script starts `set -euo pipefail`** — proper files *and* inline ansible `shell:` blocks, which must also declare `executable: /bin/bash` (`pipefail` is a bashism; ansible's default `/bin/sh` is dash and errors on it). For ansible `shell:` tasks this is enforced by the custom **`shell-strict-mode`** ansible-lint rule ([lint/ansible_rules/shell_strict_mode.py](lint/ansible_rules/shell_strict_mode.py), wired via `rulesdir` in [.ansible-lint](.ansible-lint)); built-in `risky-shell-pipe` only covers the pipefail half. Test scaffolding (`_verify*.yml` / `_setup*.yml`) is **exempt** — those probes deliberately exercise failure paths (non-zero rc, mid-pipeline failures) where `set -e` would be wrong (canonical: `bash/_verify.yml`'s `f_rescue` assertion). `-e` obliges you to handle commands you *expect* to fail: append `|| true` to a command whose non-zero exit is normal (e.g. `snap list` exiting 1 when empty), and avoid `... | head`/`... | sort | head` pipelines where the early reader raises SIGPIPE upstream — collapse the selection into a single non-early-exiting filter (one `awk` over the whole stream) instead. Canonical: the snap-purge block in [roles/cleanup/tasks/main.yml](roles/cleanup/tasks/main.yml).
...
- **Don't pin a config value just because it equals the upstream default.** A `foo: <default>` line justified only by "this is the upstream default, stated explicitly" is churn — a maintenance point that reads as meaningful when it isn't. Pin (and comment) a value only when it's *load-bearing*: it documents a dependency the rest of the system relies on (headscale's `write_ahead_log: true` is required because the DB sits on a 4K-recordsize dataset, not because it's the default), it's a deliberate *non-default*, or it's a tuning knob worth surfacing. Otherwise inherit the default silently.
- Every file-writing task (`copy:` / `template:` / `replace:` / `lineinfile:` / `blockinfile:` / `ini_file:` / `assemble:`) sets **`backup: true`** for on-disk traceability of what changed and when. This is enforced by the custom **`require-backup`** ansible-lint rule ([lint/ansible_rules/require_backup.py](lint/ansible_rules/require_backup.py), wired via `rulesdir`); test scaffolding (`_verify*`/`_setup*`) and `_`-prefixed fixture roles are exempt. There's no credential-exposure concern: ansible's `preserved_copy` reproduces the original's exact mode **and** ownership (`copy2` + `os.chown` + SELinux/attrs), so a `0600` secret-bearing file's backup stays `0600` same-owner — never widened. And no disk-bloat concern: `roles/cleanup`'s `prune_ansible_backups` daily timer deletes backups older than a week. Genuine exceptions carry `# noqa: require-backup` — the legitimate ones are writes where a backup can't or shouldn't exist: binary/asset installs (reproducible from the role), `/run` transients, restore-from-backup steps, secret-staging files (a backup would *persist* the secret), and the vfat ESP (`/boot/efi`, where `preserved_copy`'s chown fails).
...
Ports each host-level service publishes live in `group_vars/all/main.yml` under `service_ports:` — single source of truth for unit templates, nginx `proxy_pass`, firewall rules, and cross-role consumers (e.g. pihole's dnscrypt upstream). Covers both loopback-published podman services (`--publish 127.0.0.1:{{ service_ports.<svc> }}:<container_port>/tcp` + `nginx_site_args.proxy_pass: http://127.0.0.1:{{ service_ports.<svc> }}/`) and native daemons whose port is referenced by the firewall or another role. Migrate roles to consume from there as you touch them — see [notes/SOMEDAY.md](notes/SOMEDAY.md) *Migrate remaining service roles to the `service_ports` registry*. **When allocating a new port, grep both `service_ports:` and `127.0.0.1:` literals in `roles/*/templates/*.j2` first** — un-migrated roles still hard-code their port, so the registry-only check returns false negatives.
...
Everything HA reads from `/mnt/services/homeassistant/` that's user-authored — `automations.yaml`, `scripts.yaml`, `scenes.yaml` (GUI-written), plus `sensors.yaml`, `templates.yaml`, `input_numbers.yaml`, `timers.yaml`, `custom_templates/macros.jinja`, and `blueprints/automation/*.yaml` (operator-authored) — lives in a separate GitHub repo ([adrienkohlbecker/homelab_ha_config](https://github.com/adrienkohlbecker/homelab_ha_config), private) registered as a git submodule at [roles/homeassistant/files/ha_gui_config/](roles/homeassistant/files/ha_gui_config). The submodule is the source of truth. The homeassistant role only creates the directories + touches the include-targets (automations/scripts/scenes/sensors/templates/input_numbers/timers) so HA's loader doesn't error on a fresh host before the first sync (a missing stub sends HA into recovery mode, where it discards configuration.yaml entirely). Every push parse-validates each changed file (YAML via PyYAML with HA tag stubs, .jinja via jinja2) and aborts the upload on any syntax error — so a broken file never reaches lab.

Drive both directions with `mise run ha:sync [pull|push|sync]` ([mise-tasks/ha/sync.py](mise-tasks/ha/sync.py)). Preview a push first with `mise run ha:sync push --dry-run`: it diffs the host against your working tree (uncommitted edits included), runs the syntax validation, and prints what it *would* upload + reload — without writing to the host, moving the synced tag, or reloading HA. A movable tag `last_synced_to_host` on the submodule pins "the commit whose tree matches lab as of last sync"; push refuses if the host's blobs don't match that tag (host edited since last pull) and tells the operator to pull first. Each tracked file declares its reload service in `SYNC_SPEC` at the top of the script (e.g. `automations.yaml` → `automation.reload`, `templates.yaml` → `template.reload`, `custom_templates/*` → `homeassistant.reload_custom_templates`); files where no per-domain reload exists (currently `sensors.yaml` because of the `statistics` platform) are marked `None` and trigger a full `homeassistant.restart`. If any changed file is in the restart bucket, the script restarts HA once and skips per-domain reloads (covers everything). Auth via `HA_API_TOKEN` (1P-backed `[env]` entry in [mise.toml](mise.toml); long-lived token minted in HA UI → Profile → Security). Without it the push lands the YAML on disk and prints a warning; reload/restart manually.
...
`notes/` is a **git submodule** → private repo [adrienkohlbecker/homelab_notes](https://github.com/adrienkohlbecker/homelab_notes), split out so long-form design notes (often household-PII-adjacent) stay out of this **public** parent. The parent stores only a gitlink (one commit SHA); the notes content lives **only** in `homelab_notes`. The `git subtree split` that created it was one-time history surgery — this is **not** a subtree. **Never run `git subtree add/pull/push/merge`** against `notes/`; those belong to the embedding model we didn't pick and mixing them corrupts the setup.

Sync is the standard two-step submodule dance, because the notes repo and the parent pointer are separate:
1. Edit inside `notes/` (its own git repo): `cd notes && git commit … && git push` → `homelab_notes`.
2. Record it in the parent: `git add notes && git commit` (bundle into a related parent commit, or a lone `notes: bump pointer`) → public `homelab`.

On a fresh clone, `git submodule update --init` checks out the recorded SHA; `git submodule update --remote notes` advances to the latest `homelab_notes/main` (then `git add notes` + commit to record it). **Unlike `ha_gui_config`, the `notes` pointer is load-bearing and carries no `ignore = all`** — pointer divergence and a dirty `notes/` worktree *do* surface in parent `git status`, and you *should* deliberately commit pointer bumps (the exact opposite of the HA submodule above). Don't let the pointer lag forever: a fresh clone only sees notes as of the last recorded SHA. The SHA + timestamp land in the public repo, but notes *content* never does — never inline notes back into the parent tree.

**In a worktree, this is automated by the `worktree:` tooling** ([mise-tasks/worktree/](mise-tasks/worktree/), also driven by the `WorktreeCreate`/`WorktreeRemove` hooks in [.claude/settings.json](.claude/settings.json), so it fires for agent `isolation: worktree` worktrees too). `worktree:populate` inits the `notes` submodule and puts it on a branch matching the parent worktree's branch — so notes edits in the worktree commit onto `<name>`. `worktree:merge` keeps history **linear whenever it safely can**, falling back to a merge commit only when notes genuinely diverged. Three cases: a **code-only** worktree (or one whose notes commits **fast-forward** `notes/main`) is rebased onto master and FF'd — the notes branch is FF-pushed, so no merge commit and **no SHA change**, and the gitlinks the parent already records stay valid as-is. Only when `notes/main` has **diverged** (a concurrent worktree merged notes meanwhile, so the branch can't FF) does it merge the notes branch into `notes/main` (`--no-ff`) and pin master's gitlink to that merge commit via a `--no-ff` parent merge (`merge --no-commit` + force — no throwaway "bump gitlink" commit on the branch). That divergent path is why the workflow is merge-capable at all: the merged notes tip must carry *every* worktree's notes — FF'ing one branch would drop the others, and recording a single worktree's pre-merge tip on master would regress notes another worktree already landed. A same-file notes edit across two worktrees conflicts at the notes merge (or a code clash at the parent merge) and halts for manual resolution, leaving the main worktree mid-merge. Mechanics that still bite if you step outside the tooling: each worktree gets its **own** submodule clone (`.git/worktrees/<wt>/modules/notes`), independent of the main checkout — notes edits reach the main checkout only via `origin`, never cross-worktree directly; a **detached** worktree (agent isolation, `new==base`) has no parent branch so notes stays detached on the recorded SHA and `worktree:merge` skips the notes step; and since `homelab_notes` is private, the init only succeeds with SSH access (a stranger cloning the public parent gets an empty `notes/` — intended).
