...
# USAGE flag "--promote" help="After upload, update /homelab-ci/qemu-image/<machine>/<ubuntu> to this build id"
...
def build_manifest(
    *,
    ...
    return {
        ...
        "s3_prefix": s3_prefix,
        "ssm_parameter": f"{SSM_PREFIX}/{args.machine}/{args.ubuntu}",
...
def assert_new_object(bucket: str, key: str, region: str) -> None:
    if object_exists(bucket, key, region):
        sys.exit(f"refusing to overwrite existing S3 object: s3://{bucket}/{key}")


def upload_file(path: Path, bucket: str, key: str, region: str, content_type: str | None = None) -> None:
    cmd = [
        "aws",
        "--region",
        region,
...
def promote(args: argparse.Namespace) -> None:
    ...
    previous = prev.stdout.strip() if prev.returncode == 0 else ""
    if previous:
        print(f"==> previous {param}: {previous}")
    print(f"==> promoting {param} -> {args.build_id}")
    run(
        [
            "aws",
            "--region",
    ...
    if previous:
        print(
            f"    Rollback: aws --region {args.region} ssm put-parameter --name {param} --type String --value {previous} --overwrite"
        )
...
def main() -> int:
    ...
    if args.dry_run:
        ...
        if args.promote:
            print(f"DRY RUN: would promote {manifest['ssm_parameter']} -> {args.build_id}")
        return 0
    ...
    with tempfile.TemporaryDirectory(prefix="packer-s3-", dir=os.environ.get("TMPDIR")) as tmp:
        ...
        upload_file(bundle, args.bucket, bundle_key, args.region, "application/zstd")
        upload_file(manifest_path, args.bucket, manifest_key, args.region, "application/json")

    if args.promote:
        promote(args)
    else:
        print("==> upload complete; promote with:")
        print(
            f"    aws --region {args.region} ssm put-parameter "
            f"--name {manifest['ssm_parameter']} --type String --value {args.build_id} --overwrite"
        )
    return 0
